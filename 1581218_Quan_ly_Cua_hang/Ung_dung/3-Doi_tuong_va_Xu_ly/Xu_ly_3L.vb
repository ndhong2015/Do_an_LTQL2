...
Public Class XL_NGHIEP_VU
    ...
    End Function
    Public Function Tong_so_luong_Ton(Danh_sach_Tivi As List(Of XL_TIVI)) As Long
        Dim Tong As Long
        Tong = 0
        For Each Tivi As XL_TIVI In Danh_sach_Tivi
            Tong += Tinh_So_luong_Ton_Tivi(Tivi)
        Next
        Return Tong
    End Function
    Public Function Tinh_So_luong_Ton_Nhom_Tivi(Nhom_Tivi As XL_NHOM_TIVI, Du_lieu As XL_DU_LIEU) As Long
        ...
        Return So_luong_Ton
    End Function
    Public Function Danh_sach_Tivi_theo_nhan_vien(Nguoi_dung As XL_NHAN_VIEN_BAN_HANG, Danh_sach_Tivi As List(Of XL_TIVI)) As List(Of XL_TIVI)
        Dim Danh_sach_Tivi_theo_NV = New List(Of XL_TIVI)
        Danh_sach_Tivi.ForEach(
            Sub(Tivi)
                ...
                If Duoc_Phan_quyen Then
                    Danh_sach_Tivi_theo_NV.Add(Tivi)
                End If
            End Sub)
        Return Danh_sach_Tivi_theo_NV
    End Function
    Public Function Tinh_Doanh_thu_Ngay_Hien_hanh_Tivi(Tivi As XL_TIVI) As Long
        ...
        Return Doanh_thu
    End Function
    Public Function Tong_Doanh_thu_Ngay(Danh_sach_Tivi As List(Of XL_TIVI)) As Long
        Dim Doanh_thu As Long
        Doanh_thu = 0
        For Each Tivi As XL_TIVI In Danh_sach_Tivi
            Doanh_thu += Tinh_Doanh_thu_Ngay_Hien_hanh_Tivi(Tivi)
        Next
        Return Doanh_thu
    End Function
    Public Function Tinh_Doanh_thu_ngay_Nhan_vien(Nhan_vien As XL_NHAN_VIEN_BAN_HANG, Danh_sach_Tivi As List(Of XL_TIVI)) As Long
        Dim Doanh_thu = Danh_sach_Tivi.Sum(Function(Tivi) Tinh_Doanh_thu_Ngay_Hien_hanh_Tivi(Tivi))
        Return Doanh_thu
    End Function
End Class
'**************** Xử lý  Lưu trữ *******************
...
#Region "Xử lý Ghi"
    Public Function Cap_nhat_Don_gia_Nhap_Tivi(Tivi As XL_TIVI, Don_gia_Nhap_Moi As Long) As String
        ...
        Return Kq
    End Function
    Public Function Cap_nhat_Don_gia_Ban_Tivi(Tivi As XL_TIVI, Don_gia_Ban_Moi As Long) As String
        Dim Kq As String = ""
        Dim Don_gia_Ban_Hien_nay = Tivi.Don_gia_Ban
        Tivi.Don_gia_Ban = Don_gia_Ban_Moi
        Try
            Dim Duong_dan = Thu_muc_Tivi.FullName & "\" & Tivi.Ma_so & ".json"
            Dim Chuoi_JSON = JsonConvert.SerializeObject(Tivi)
            File.WriteAllText(Duong_dan, Chuoi_JSON)
        ...
        End Try
        If Kq <> "OK" Then
            Tivi.Don_gia_Nhap = Don_gia_Ban_Hien_nay
        End If
        Return Kq
    End Function
#End Region
